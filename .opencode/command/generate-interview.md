...
Usage: `/generate-interview <language> <role> <level>`

- **$1** (language): python-fastapi, nodejs-express, csharp-dotnet, java-springboot, go-gin, ruby-rails, typescript-nestjs
- **$2** (role): backend, fullstack, devops, security, accessibility
- **$3** (level): junior, mid, senior, staff

Example: `/generate-interview python-fastapi backend senior`
...
1. **Create the interview branch:**
   ...
   git checkout main
   git pull
   git checkout -b interview/$1-$2-$3
   ```
...
2. **Commit the implementation:**
   ```bash
   git add .
   git commit -m "Add $1 implementation for $2 ($3)"
   git push -u origin interview/$1-$2-$3
   ```
